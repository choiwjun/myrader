...
> 소상공인 모듈의 boina 내 통합은 04 문서 §5. 이 문서는 공유 패키지와 **미래 Creator Radar 별도 레포**를 다룬다. 현재 `apps/web` SME v1 구현 범위에는 Creator 화면, 유료 결제, package publish/release workflow가 포함되지 않는다.
...
| 결제 | 토스페이먼츠 빌링 | 미래 Creator/유료 구독 범위. 현재 SME v1의 `radar_subscriptions`는 결제가 아닌 무료 `trialing/active` 스캔 예약 리소스 |
| LLM | Claude API (프로브·해석문). 엔진 llm-provider 라우터 경유 | 서킷브레이커·리트라이 기제공 |
| 메일 | Resend (주간 리포트·인용 알림·매직링크) | 발송 실패 로깅 + 재시도 1회 |
| 배포 | Vercel + GitHub Actions | 앱 CI/CD 기준. 현재 패키지 발행용 release workflow는 추가하지 않음 |
...
│   └── billing/                  # 미래 Creator 유료 구독 범위
├── db/schema/                    # Drizzle (§3)
├── jobs/                         # scan.daily / probe.weekly / report.weekly
└── packages 소비: 현재 SME v1은 workspace packages, Creator 별도 레포는 미래 publish 이후 @boina/*/@radar/* 소비
```

**공유 패키지 (G005 정렬)**
- 현재 SME v1: `packages/engine`, `packages/contracts`, `packages/keyword-pipeline`을 모노레포 워크스페이스 TS 소스 패키지로 소비한다. GitHub Packages 발행, dist build, package release workflow는 현재 필수조건이 아니다.
- 미래 Creator 별도 레포: dist 산출물, package exports, 버전 태그, release workflow가 준비된 뒤에만 `@boina/engine`, `@boina/contracts`, `@radar/keyword-pipeline` 발행·소비로 전환한다. `@radar/keyword-pipeline` export 목표는 `expand(seed, opts)`(시드 분해 폴백 내장), `collectSignals(keywords)`(블로그/데이터랩/검색광고), `naverScore(signals)`.

## 3. DB 스키마 (Drizzle 의사 표기)

```
users            id, email, plan(free|starter|pro), created_at
billing          user_id, toss_billing_key, status(active|past_due|canceled), next_charge_at  -- 미래 Creator 유료 구독 범위
...
POST /api/billing/checkout          미래 Creator 유료 구독: 토스 빌링키 발급/첫 결제 진입
POST /api/billing/cancel            미래 Creator 유료 구독: 1클릭 해지, 다음 결제일까지 이용 가능
POST /api/billing/webhook           미래 Creator 유료 구독: 토스 웹훅 수신(서명 검증+멱등 처리)
GET  /api/billing/status            미래 Creator 유료 구독: 현재 플랜/결제 실패/유예 상태
```
...
- 탈퇴: `DELETE /api/me` — 30일 유예 소프트 삭제 후 하드 삭제(개인정보·프로브 응답 원문 포함). 미래 유료 구독이 활성 상태면 해지 선행 강제. S7 설정에 노출.
- 결제 상태 전이는 미래 유료 구독 착수 시 웹훅을 기준으로만 확정한다. 프론트 redirect 성공은 UI 힌트일 뿐이며, DB 반영은 서명 검증된 웹훅과 멱등 키로 처리한다.
...
- [ ] 미래 유료 구독 착수 시 토스 웹훅 서명 검증, 결제 상태는 웹훅만 신뢰
- [ ] 미래 유료 구독 착수 시 토스 웹훅 멱등 처리(`paymentKey`/이벤트 ID 중복 방지), 실패 유예 3일, 무료 플랜 강등 정책 검증
- [ ] 미래 유료 구독 착수 시 해지/환불 정책 문구와 실제 결제 상태 전이 일치
