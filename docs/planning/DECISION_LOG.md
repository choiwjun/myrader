...
- **정보 계층(전 화면·데이터 소스 우선순위)**: ①**AI 노출(HERO)** — "AI가 너를 추천하나" (grounded llmValidation) → ②**라이벌 비교·역공학** — "경쟁사는 되는데 너는? 걔는 어떻게?" (GapAnalyzer) → ③**레버 채널: 네이버 > 구글(v1.5)** — "AI가 널 찾게 하는 연료". SNS/인스타·블로그·유튜브 자동 연동은 현재 SME v1 채널이 아니라 future lever scope.
...
- 반영: 목업(design/mockup), 05-design-system(정보 계층), 02-trd(데이터 소스 우선순위), 06-screens(S2/S3 순서·SME v1 채널 taxonomy `naver/google/ai`) patch.
