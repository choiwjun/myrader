...
> 목표: 모노레포 골격 + 엔진 통합 자리 + DB + 백그라운드 잡 골격. **OQ-6은 현재 workspace-only 패키지 사용으로 해소, OQ-5 결정은 이 Phase에서 명시.**
...
**Phase 0 게이트:** 앱 부팅 + 엔진 타입 import + DB 마이그레이션 + 더미 잡 완주 = Green. OQ-5는 `[OPEN]` 유지 허용(베이스라인 차단 아님, ADR 기록 필수). OQ-6은 현재 workspace-only로 닫혔으며 외부 발행은 현재 게이트가 아니다.
...
| OQ-6 | 엔진 재사용 방식 | P0-R0 | **해소:** 현재 SME v1은 private workspace-only TS 소스 패키지 import. GitHub Packages 발행은 dist build·release workflow 준비 후 별도 결정 |
...
- **ICV:** 누락 0건 — 통과(§4).
- **Open questions:** OQ-1(제품명)·OQ-3(가격)·OQ-4(SERP 키)·OQ-5(잡 인프라) — 해당 태스크 `[OPEN]`. OQ-2 확정. OQ-6은 현재 SME v1 기준 workspace-only 패키지 사용으로 해소.
...
- **Risks:** 비IT 사장님 이해 실패(최대 베팅) / workspace-only 엔진 경계 누수 / 데이터 비용(SERP·AI 게이팅) / 잡 인프라 과·소설계(OQ-5) / 가격 미검증(OQ-3)
