...
| 포함 (v1 ✅) | 보류 (v1.5 ⏭) | 영구 제외 (🚫) |
|---|---|---|
| 가게 찾기(네이버 플레이스) → 내 상태(네이버 실측 + on-page SEO/AEO/GEO + AI 인용 샘플) → 경쟁 비교 → **역공학 갭(핵심 차별)** → 행동 4분류 + "오늘 딱 하나" → 쉬운 생성물 | 구글 실 SERP 순위(SerpAPI 키 OQ-4), 재진단/추이 모니터링, grounded AI 대량측정 | 결제, 구독, Kakao/SMS 알림, 대행/매칭 마켓플레이스, 외부 업체 중개·정산, 매출/스마트플레이스 성과연결 |
...
- **프론트 Screen 태스크 `P{N}-S{M}` = 신규 빌드.** Next.js App Router 7화면. 비IT 사장님 UX(신호등·전문용어 0·모바일·한 번에 하나·응원 톤·페이월 경계)로 새로 짓는다.
...
- **[OPEN] OQ-6 — 엔진 재사용 방식(import vs 추출/복사) 미결정.** P0-R0에서 결정. 결정 전까지 **workspace import 전제**로 작성, 결정 후 07 §2에 명문화.
...
**결과: 누락 0건 — ICV 통과.** 모든 화면 데이터 요구가 리소스로 충족되고, 모든 리소스가 x-sag 계약으로 추적된다(resources.yaml `source` 필드). PlanTier(페이월)는 OQ-3 가격 OPEN이나 free/paid 경계 자체는 계약됨.
...
> 목표: 모노레포 골격 + 엔진 통합 자리 + DB + 백그라운드 잡 골격. **OQ-6/OQ-5 결정을 이 Phase에서 명시.**

| ID | 태스크 | 담당 | REQ | 헌법 | 병렬 | Depends On |
|---|---|---|---|---|---|---|
| P0-T1 | 모노레포 셋업 | backend | — | — | 시작점 | — |
| P0-R0 | x-sag 엔진 통합 셋업 [OPEN OQ-6] | backend | 전체 | 07 §2 | P0-T1 후 | P0-T1 |
| P0-T2 | DB(Postgres+Drizzle) 스키마 차용 | database | 전체 | common/uuid, common/seed-validation | P0-R0 병렬 | P0-T1 |
| P0-T3 | 백그라운드 잡 골격 [OPEN OQ-5] | backend | REQ-002 | nextjs/api-routes | P0-T2 후 | P0-T2, P0-R0 |
...
- [ ] **P0-R0 x-sag 엔진 통합 셋업 [OPEN: OQ-6 — import vs 추출/복사]** — backend specialist
  - 내용: x-sag `core-engine`(크롤·파서·analyzers·scoring·v2: gap/competitor/serp/geo-validator/nlp) + `contracts`(diagnosis/api 타입)를 `packages/engine`·`packages/contracts`로 가져온다. **[OPEN OQ-6]** 가져오는 방식 — (a) workspace 패키지 그대로 import vs (b) 추출/복사 — **미결정**. 결정 전까지 (a) import 전제로 배선하고, 결정 후 `07-coding-convention.md §2`에 명문화. `naver-presence`·`llm-provider`·`snippet` 로직도 통합 범위.
  ...
  - 발명 금지: OQ-6 미결정을 태스크 산출물(README/ADR)에 `[OPEN]`으로 남긴다. 방식 임의 확정 금지.
...
- [ ] **P0-T3 백그라운드 잡 골격 [OPEN: OQ-5 — 잡 인프라 선택]** — backend specialist
  - 내용: 느린 분석(크롤·SERP·AI 인용)을 동기 처리하지 않도록 잡 큐 골격. enqueue → worker → status(`queued|running|done|failed`) → diagnosis 반영. **[OPEN OQ-5]** 인프라 — BullMQ vs 경량 큐/서버리스 — **미결정**. 교체 가능하도록 **추상화 레이어(JobQueue 인터페이스)** 뒤에 둔다. 비용 게이팅 함수 자리(grounded llmValidation·SERP 무분별 호출 금지) 포함.
  - 헌법: `nextjs/api-routes.md`(enqueue Route Handler 리소스 중심), `02-trd §3`(상태 모델).
  - TDD: RED 더미 잡 enqueue 시 status 전이 안 됨 → GREEN queued→running→done 전이 + 실패 시 failed 통과 → REFACTOR JobQueue 인터페이스 추출(인프라 교체 대비).
  - 발명 금지: OQ-5 미결정을 ADR에 `[OPEN]`. 인프라 임의 확정 금지(추상화 뒤에 더미 구현으로 시작).
  - 병렬: P0-T2 + P0-R0 후.

**Phase 0 게이트:** 앱 부팅 + 엔진 타입 import + DB 마이그레이션 + 더미 잡 완주 = Green. OQ-5/OQ-6은 `[OPEN]` 유지 허용(베이스라인 차단 아님, ADR 기록 필수).
...
- [ ] **P2-R4 gapItem (x-sag GapAnalyzer 통합·배선 — 잠들어 있던 것)** — backend specialist
  ...
  - TDD: RED GapAnalyzer 미배선/룰 코드값 노출 → GREEN GapMatrixRow→사장님 label + priority 정렬 + isPaid 경계 통과 → REFACTOR 번역 사전 분리.
  - 병렬: P2-R3 후. R5·R6의 선행.

- [ ] **P2-R5 action (4분류 + "오늘 딱 하나")** — backend specialist
  - 내용: `action`(id, title, tier, isTodayOne, deeplink, doneable, isPaid) — GapAnalyzer actionType/priority → **사장님 언어 4분류 번역**(green_self/yellow_copy/red_vendor/gray_ongoing). PriorityGap 활용 **"오늘 딱 하나"(isTodayOne=true, 1개)** 우선순위 로직(신규). 직접건(green_self)은 deeplink. isPaid로 [무료] 오늘딱하나+일부 / [유료] 전체.
...
- [ ] **P2-R6 generatedAsset (snippet 생성 통합 — [유료] 실행팩)** — backend specialist
  - 내용: `generatedAsset`(id, type, title, content, copyable) — **x-sag snippet 생성 통합**. type 4종: snippet(FAQ/스키마=검색 답변글), place_intro(플레이스 소개글), review_request(리뷰 요청 문구), vendor_prescription(업체 처방전 이메일초안). copyable 항상 true. 전체는 [유료] 실행팩. **생성물도 카피 가드 통과**(인과·과장 금지) 후 출력.
...
- [ ] **P2-S4 reverse-gap (/gap · auth: false)** — frontend specialist
  - 컴포넌트: gap_intro(정직한 한 문장, 보장 아님) / gap_matrix_card(옆집 ⭕/우리 ✕, priority 오름차순, **[무료] Top3만**, 클릭 시 /actions) / paid_gap_lock(**[유료] 경계**, Top3 아래 잠금+손실 프레이밍, unlock→paywall_gate) / go_actions_button. 데이터: gapItem.
  - UX 수용기준: 사장님 언어 label(룰 코드값 0) / 인과 단정 금지(위생·구조 격차) / competitorTop과 구분 / 점수 0. AC-7 게이트 / AC-8.
  - 페이월: [무료]=priority Top3, [유료]=전체 매트릭스(P3에서 배선). **[OPEN OQ-3]** 가격 placeholder.
  - 헌법: `07 §4`(점수·인과 가드), `tailwind/v4-syntax.md`.
  - TDD: RED 전체 노출(무료)/룰 코드값 → GREEN 무료 Top3+나머지 잠금+사장님 label 통과 → REFACTOR.
  - 병렬: P1-S0·P2-R4 후.

- [ ] **P2-S4-V S4 연결점 검증** — test specialist
  - navigations: gap_matrix_card·go_actions_button → /actions. shared: paywall_gate. external: GapAnalyzer. 무료 Top3/유료 전체/잠금 해제 진입. AC-8.

- [ ] **P2-S5 actions (/actions · auth: false)** — frontend specialist
  - 컴포넌트: today_one("오늘 딱 하나" 배너 최상단, 1개 크게, [무료] 항상 노출, deeplink/상세) / green_self_card(🟢 5분 직접+deeplink) / yellow_copy_card(🟡 복붙→/assets) / red_vendor_card(🔴 업체 처방전→/assets) / gray_ongoing_card(⏳ 꾸준히, 조급 금지) / paid_actions_lock(**[유료] 경계**, unlock→paywall_gate). 데이터: action.
  - UX 수용기준: 한 번에 하나(오늘 딱 하나 강조) / 4분류 색·아이콘 고정 / 응원 톤 / 인과 금지("도움이 돼요"). AC-4(4분류+오늘 딱 하나) / AC-5(직접건 딥링크) / AC-8 / AC-7 게이트.
  - 페이월: [무료]=오늘딱하나+일부, [유료]=전체(P3). **[OPEN OQ-3]** 가격 placeholder.
...
- [ ] **P2-S5-V S5 연결점 검증** — test specialist
  - navigations: yellow_copy_card·red_vendor_card → /assets. shared: today_one_banner, paywall_gate. external: diagnosis_engine(action), naver_place(deeplink). 무료/유료 경계·딥링크. AC-4/AC-5.

- [ ] **P2-S6 generated (/assets · auth: false)** — frontend specialist
  - 컴포넌트: assets_intro("그대로 복사해서 쓰시면 돼요") / snippet_card(검색 답변글) / place_intro_card(가게 소개글) / review_request_card(리뷰 요청 문구) / vendor_prescription_card(업체 처방전 이메일) — 각 **big_copy_button** / paid_assets_lock(**[유료] 경계**, 무료=미리보기/일부, unlock→paywall_gate). ?type로 S5 특정 생성물 진입. 데이터: generatedAsset.
  - UX 수용기준: 전문용어 0("snippet"→"검색 답변글") / 큰 복사 버튼+"복사됐어요" / 효과 보장 단정 금지. AC-6(복붙 가능 형태) / AC-7 게이트.
  - 페이월: [유료] 실행팩=생성물 전체(P3). **[OPEN OQ-3]** 가격 placeholder.
...
- [ ] **P2-S6-V S6 연결점 검증** — test specialist
  - shared: big_copy_button, paywall_gate. external: diagnosis_engine(snippet). 복사 동작·?type 진입·유료 경계. AC-6.
...
> 2026-07-05 사용자 지시로 Toss 결제와 Kakao/SMS 알림은 현재 개발 범위에서 제외한다. 기존 paywall/결제 태스크는 구현 대상이 아니라 legacy risk 정리 대상으로 둔다.
...
- [ ] **P3-S1 결제 UI 제거/redirect** — frontend specialist
  ...
  - TDD: RED unlock 후 결제 진입 안 됨/결제 후 잠금 유지 → GREEN unlock→결제→전체 노출 통과 → REFACTOR.
...
```mermaid
graph TD
  P0T1[P0-T1 모노레포] --> P0R0[P0-R0 엔진통합 OQ-6]
...
- [ ] **(scope out) 결제/구독/외부 알림** — 현재 범위에서 제외. 재도입하려면 별도 사용자 결정과 문서 갱신이 필요하다.
- [ ] **(v1.5) grounded AI 대량측정** — v1은 인용 샘플·게이팅만.

---

# Open Questions (발명 금지 — 해당 태스크에 [OPEN] 명시)

| OQ ID | 미결정 항목 | 영향 태스크 | 처리 |
|---|---|---|---|
| OQ-1 | 제품명/브랜딩 | P1-S0(app_header) | 헤더 브랜드 영역 placeholder |
| OQ-3 | 무료/유료 경계·가격 액수 | P3-R1 | 현재 결제 제외. 경계 계산 호환만 유지하고 금액/결제수단은 확정하지 않음 |
| OQ-4 | 구글 SERP 키·비용 | (v1.5) google 채널 | v1 맛보기만, 실 SERP 보류 |
| OQ-5 | 백그라운드 잡 인프라 | P0-T3 | JobQueue 추상화 뒤 더미로 시작, ADR `[OPEN]` |
| OQ-6 | 엔진 재사용 방식(import vs 추출) | P0-R0 | import 전제로 작성, 결정 후 07 §2 명문화 |
...
| **AC-7 사장님 이해도 테스트(✅ 충족 2026-06-14)** | 비IT 사장님 2~3명, 5분 내 "내 상태/경쟁/할 일" 자력 설명 — 2026-06-13 목업 검증(사용자 보고, 목업 리뷰 기반) | 01 AC-7·05 §6 |
...
- **ICV:** 누락 0건 — 통과(§4).
- **Open questions:** OQ-1(제품명)·OQ-3(가격)·OQ-4(SERP 키)·OQ-5(잡 인프라)·OQ-6(엔진 통합 방식) — 해당 태스크 `[OPEN]`. OQ-2 확정.
- **Assumptions:** x-sag 엔진 부품 재사용 가능 / Naver Search API 접근 가능 / Postgres+Drizzle 재사용 가능 / 비IT 사장님 모바일 단독 사용
- **Validation criteria:** 진단 잡 비동기 완주 + 신호등 반환 / 점수·인과·전문용어 카피 UI 0건 / 무료·유료 경계 서버 강제 / 헌법 위반 0 / **AC-7 사장님 이해도 테스트 ✅ 충족(2026-06-14, 목업 검증)**
- **Risks:** 비IT 사장님 이해 실패(최대 베팅) / 엔진 결합 강도(OQ-6) 경계 누수 / 데이터 비용(SERP·AI 게이팅) / 잡 인프라 과·소설계(OQ-5) / 가격 미검증(OQ-3)
