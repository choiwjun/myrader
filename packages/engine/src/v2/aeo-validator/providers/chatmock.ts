/**
 * X-SAG Core Engine v2 — ChatMock AEO Validator Provider
 *
 * ChatMock(OpenAI 호환 로컬 프록시)에 AEO 질의를 전송하고 응답을 분석한다.
 *
 * 활성화 조건 (둘 중 하나):
 *   1. CHATMOCK_ENABLED=true
 *   2. CHATMOCK_BASE_URL 명시적 설정
 *
 * 환경변수 (geo-validator/providers/chatmock.ts 와 공유):
 ...
 *   - max_tokens: 600 (AEO 답변은 GEO 보다 짧음)
 *   - temperature: 0.3
 *   - timeout: 30s
...
export interface ChatMockAeoValidatorOptions {
...
export class ChatMockAeoValidator implements AeoValidator {
	...
	constructor(options: ChatMockAeoValidatorOptions = {}) {
	...
	async validate(
		...
		for (let i = 0; i < qs.length; i++) {
			const q = qs[i];
			if (!q) continue;
			const llmResponse = await this.askChatMock(q.query);
			const citation = analyzeAeoCitation(q, llmResponse, input);
		...
		const metrics = computeAeoMetrics(citations);
	...
	private async askChatMock(query: string): Promise<string> {
			...
			messages: [
				{
					role: "system",
					content:
						"You are a Korean expert assistant. Answer the user's information request directly. If you can recommend specific businesses or sources, mention them by name with locations when relevant. Be informative.",
				},
		...
		try {
			...
			// 회로 open 또는 재시도 소진된 일시적 에러/네트워크/타임아웃 —
